...
if(i){this._invisibleParent=i;
this._onParentVisibilityChangeDelegate=Function.createDelegate(this,this._initializeSliderControl);
this._addParentVisibilityChangeHandler(i,this._onParentVisibilityChangeDelegate);
...
return z;
},_playAnimation:function(k,p){if(this._animationEnded==false){this._handleInSlidingMode=this._animatedHandle;
return;
}this._animationEnded=false;
var o=this;
var m=function(){o._setActiveDragHandleValue(k,p,true);
o._animationEnded=true;
};
var j=this._isHorizontal;
var f=this._animationDuration;
var l=this._calculateDragHandleOffset(k);
var g=this._getSelectionAnimationEndBounds(l,p);
var n=j?{width:g.width+"px",left:g.x+"px"}:{height:g.height+"px",top:g.y+"px"};
a(this._selectedRegionElement).stop().animate(n,f,null,m);
if(this._showDragHandle){var i=this._animatedHandle=p?this._dragHandleElement:this._endDragHandleElement;
...
},_setHandlePosition:function(h,g,k,j,f){if(!j&&this.raise_valueChanging(h,k)){return;
}var e=g;
if(!e){e=this._handleInSlidingMode;
}if((this._animationDuration>0)&&f){this._playAnimation(h,k);
}else{var i=this._calculateDragHandleOffset(h);
if(this._showDragHandle&&e){e.style[this._getLocationProperty()]=i+"px";
this._updateRelativeLiveDragHandlePosition(e);
}this._setActiveDragHandleValue(h,k,true);
this._updateSelectedRegion(e,i,k);
...
},_setActiveDragHandleValue:function(h,n,k,l,m){var e=n?this._dragHandleElement:this._endDragHandleElement;
if(!e&&this._showDragHandle){return;
}var j=this._getActiveDragHandleValue(n);
if(!k&&this.raise_valueChanging(h,n)){return true;
}var f=this._isDirectionReversed;
var g=this._isSelectionRangeEnabled;
if(n){if(!m&&((h>this._selectionEnd&&g&&!f)||(h<this._selectionEnd&&g&&f))){this._value=this._selectionEnd;
...
}}else{if(!m&&((h<this._value&&g&&!f)||(h>this._value&&g&&f))){this._selectionEnd=this._value;
this._selectedRegionElement.style[this._getSizeProperty()]="0px";
this._switchDragHanldes();
this._value=h;
}else{this._selectionEnd=h;
}}if(this._itemType==b.SliderItemType.Item){this._updateSelectedItemsStatus();
}if(!l){this.updateClientState();
}if(h!=j){this.raise_valueChanged(j,h);
if(!this._inSlidingMode&&this._autoPostBack){this._raiseValueChangedServerEvent();
}}},_setNewValue:function(o,m,n){o=c.parseDecimal(o,this._precision);
if(isNaN(o)){var f=this._isSelectionRangeEnabled?(m?"SelectionStart":"SelectionEnd")+" should be of type decimal":"Value should be of type decimal";
throw Error(f);
}var i=this._minimumValue;
var h=this._maximumValue;
if(o<i){o=i;
}else{if(o>h){o=h;
}}if(!this._initializedInternal||!this._wrapperElement){if(this._initializedInternal){o=this._getNearestStepValue(o);
}this[m?"_value":"_selectionEnd"]=o;
this.updateClientState();
return;
}if(this._isSelectionRangeEnabled){var g=this._isDirectionReversed;
var l=this._value;
var k=this._selectionEnd;
if((o<l&&!m&&!g)||(o>l&&!m&&g)){m=true;
}else{if((o>k&&m&&!g)||(o<k&&m&&g)){m=false;
}}}var j=this._calcValue(o,null,null,m);
var e=this[m?"_dragHandleElement":"_endDragHandleElement"];
this._setHandlePosition(j,e,m,n);
...
(function() {
    function loadHandler() {
        var hf = $get('ctl00_MainContent_RadScriptManagerResultPage_TSM');
        if (!hf) return;
        if (!hf._RSM_init) { hf._RSM_init = true; hf.value = ''; }
        hf.value += ';;System.Web.Extensions, Version=4.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35:vi-VN:8f95decb-d716-4257-bc42-c772df7173e5:ea597d4b:b25378d2;Telerik.Web.UI, Version=2013.2.611.35, Culture=neutral, PublicKeyToken=121fae78165ba3d4:vi-VN:57877faa-0ff2-4cb7-9385-48affc47087b:16e4e7cd:ed16cbdc:f7645509:24ee1bba:874f8ea2:1e771326:f9b9258';                                    
